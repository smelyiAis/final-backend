...
async function adminControllerEditTrackPost(req, res) {
    const trackId = req.params.id;
    const { name, name_ru, description, description_ru, country, country_ru, city, city_ru, length, corners, attitude_difference } = req.body;
    const tracks = await Track.find();
    ...
    try {
        ...
        return res.render("admin_tracks", { user: req.session?.user, error: "Error editing track", success: false, tracks });
    }

    return res.render("admin_tracks", { user: req.session?.user, error: null, success: "Track has been updated", tracks });
}

async function adminControllerAddTracks(req, res) {
    let { name, name_ru, description, description_ru, country, country_ru, city, city_ru, length, corners, attitude_difference, link1, link2, link3 } =
        req.body;
    const tracks = await Track.find();
    try {
